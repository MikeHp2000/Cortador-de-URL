##Modulo para la ventana
import tkinter as tk
from tkinter import ttk
from PIL import Image, ImageTk
from tkinter import filedialog
##Modulo para el recortador
import pyshorteners as py
##Validacion para enlaces
import requests
import io
##Para hacer dataframe
import pandas as pd
##Para que se pueda copiar el URL
import pyperclip as pyp
##Para convertir en qr
import qrcode

##Funcion para reajustar el tamaño de la image
def resize_image(image_path, width, height):
  original_image = Image.open(image_path)
  resized_image = original_image.resize((width, height))
  return ImageTk.PhotoImage(resized_image)

##Funcion para pegar texto
def paste_to_entry(event=None):
    clipboard_text = root.clipboard_get()
    URL_LARGO.delete(0, tk.END)
    URL_LARGO.insert(0, clipboard_text)

##Creacion de la ventana
root=tk.Tk()
root.geometry("1000x500+30+30")
root.resizable(False, False)
root.title("Recortador de URL")

canva1=tk.Canvas(root, width=1000, height=500)
canva1.pack(fill="both", expand=True)

resized_bg=resize_image("BocchiTheRock1.jpg",1000,500)
canva1.create_image(0,0,image=resized_bg, anchor=tk.NW)


##Labels de Textos
label_url_largo=tk.Label(root, text="URL",
                         width=10, height=1,
                         relief="solid",
                         font=("Courier", 17) 
                        )
label_url_largo_1=canva1.create_window(200,200,
                                     anchor=tk.CENTER,
                                     window=label_url_largo
                                    )
##Ingresar Datos para que se pueda pegar el texto
URL_LARGO=tk.Entry(root,
                   textvariable=tk.StringVar(),
                   width=50,
                   font=("Courier",17),
                   bd=1,
                   relief="solid"
                  )
URL_LARGO_1=canva1.create_window(630,200,
                                 anchor=tk.CENTER,
                                 window=URL_LARGO
                                 )
URL_LARGO.bind(paste_to_entry)

##Boton para convertir URL largo a URL corto
def mostrar_aviso(titulo, mensaje, mensaje1,imagen=None):
    nueva_ventana = tk.Toplevel(root)
    nueva_ventana.title(titulo)
    nueva_ventana.geometry("500x500+50+50")
    nueva_ventana.resizable(False, False)
    
    #texto = tk.Label(nueva_ventana, text=mensaje, justify=tk.CENTER,  wraplength=400)
    #texto.pack()

    #texto1 = tk.Label(nueva_ventana, text=mensaje1, justify=tk.CENTER,  wraplength=400)
    #texto1.pack(side=tk.TOP, pady=50)

    if imagen:
        im=ImageTk.PhotoImage(imagen)
        label_imagen=tk.Label(nueva_ventana, image=im)
        label_imagen.image=im
        label_imagen.pack()

        ##Botones Volver y Copiar
        ButtonBack=tk.Button(nueva_ventana, text="Volver",
                         command=nueva_ventana.destroy )
        
        ButtonCopiar=tk.Button(nueva_ventana, text="Copiar",
                           command=pyp.copy(mensaje1) )

        ##Acomodo de los botones
        ButtonBack.place(x=150,y=400)
        ButtonCopiar.place(x=300,y=400)
    
    else:
       #im = ImageTk.PhotoImage(Image.open("BocchiTheRock.jpg"))
       im=resize_image("BocchiTheRock.jpg",300,300)
       label_imagen=tk.Label(nueva_ventana, image=im)
       label_imagen.image=im
       label_imagen.pack(side=tk.TOP, pady=50)

       ButtonBack=tk.Button(nueva_ventana, text="Volver",
                         command=nueva_ventana.destroy)
    
       ButtonBack.place(x=200,y=400)

    
##Declaracion de la Base de Datos
BaseDatos=pd.DataFrame(columns=["URL","URL Acortado"])

##Funcion para vaidad enlaces URL con Seguridad
def validar_enlace_http_Seguridad():
  enlace = URL_LARGO.get()
  try:
        response = requests.get(enlace)
        response.raise_for_status() 
        s = py.Shortener()
        url_acortado = s.tinyurl.short(enlace)

        global BaseDatos
        BaseDatos=pd.concat([BaseDatos, pd.DataFrame({"URL": [enlace], "URL Acortado": [url_acortado]})],
                             ignore_index=True)
        
        ##Convertir URL a imagen
        qr=qrcode.QRCode(version=1, box_size=10, border=5)
        qr.add_data(url_acortado)
        qr.make(fit=True)
        imagen_qr=qr.make_image(fill_color="black",border_color="black")
        
        mensaje = f"El enlace {enlace} es válido. Código de respuesta: {response.status_code}"
        mostrar_aviso("Aviso", mensaje, url_acortado, imagen_qr)

  except requests.exceptions.RequestException as e:
        mensaje = f"Error al acceder al enlace {enlace}: {e}"
        mensaje1 = "No se pudo acortar en URL"
        mostrar_aviso("Aviso", mensaje, mensaje1)
        URL_LARGO.delete(0, tk.END)

  except py.ShorteningErrorException as e:
        mensaje = f"Error al acortar la URL {enlace}: {e}"
        mensaje1 = "No se pudo acortar en URL"
        mostrar_aviso("Aviso", mensaje, mensaje1)
        URL_LARGO.delete(0, tk.END)
        
##Funcion para ver el historial de los URL acortados
def History():
  nueva_ventana = tk.Toplevel(root)
  nueva_ventana.title()
  nueva_ventana.geometry("900x500+20+20")
  nueva_ventana.resizable(False, False)

  ##Invocar a BaseDatos
  global BaseDatos
  ##Crear tree
  tree=ttk.Treeview(nueva_ventana, columns=("URL", "URL Acortado"), show="headings")
  ##Agregar headings al tree
  tree.heading("URL", text="URL")
  tree.heading("URL Acortado", text="URL Acortado")
  ##Ajustar tamaño de las columnas
  tree.column("URL", width=400)
  tree.column("URL Acortado", width=100)
  
  scrollbar = ttk.Scrollbar(nueva_ventana, orient="vertical", command=tree.yview)
  tree.configure(yscrollcommand=scrollbar.set)

  for index, row in BaseDatos.iterrows():
    tree.insert("", tk.END, values=(row["URL"], row["URL Acortado"]))
  
  tree.pack(side=tk.TOP, fill=tk.BOTH, expand=True, padx=10, pady=10)
  scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
  
  ButtonBack=tk.Button(nueva_ventana, text="Volver",
                         command=nueva_ventana.destroy)
  ButtonBack.pack()
  
##Botones dentro del root
button1 = tk.Button(root, text="Acortar",
                    command=validar_enlace_http_Seguridad,
                    padx=5, pady=5, width=15, height=2)
button2 = tk.Button(root, text="Historial",
                    command=History,
                    padx=5, pady=5, width=15, height=2)

def borrar_contenido(): URL_LARGO.delete(0, tk.END)

button3 = tk.Button(root, text="Borrar",
                    command=borrar_contenido,
                    padx=5, pady=5, width=15, height=2)  


##Acomodo de botones
x_spacing, A = 150, 200
canva1.create_window(A, 300, anchor=tk.W, window=button1)
canva1.create_window(A + button1.winfo_reqwidth() + x_spacing, 300, anchor=tk.W, window=button2)
canva1.create_window(A + 2 * (button1.winfo_reqwidth() + x_spacing), 300, anchor=tk.W, window=button3)

root.mainloop()


